...
for j in range(0,nely):
    for i in range(0,nelx):
        if i<nelx-1 and j<nely-1:
           ...
           vx_reconstructed_1 = rhs_x/lhs_x
           vy_reconstructed_1 = rhs_y/lhs_y

           dudx_find[node]  = (vx_reconstructed_1-u[node])/delta_x
           dvdx_find[node]  = (vy_reconstructed_1-v[node])/delta_x
        counter +=1
