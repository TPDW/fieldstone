...
#This version of Fieldstone is a test of the full internal strain method
#or whatever I'm calling it (also check how new an idea this is)
...
bc_fix_strain=np.zeros(NfemV,dtype=np.bool)  # boundary condition, yes/no
bc_val_strain=np.zeros(NfemV,dtype=np.float64)  # boundary condition, value
for i in range(0, nnp):
    if x[i]<eps:
       bc_fix_strain[i*ndofV  ] = True ; bc_val_strain[i*ndofV  ] = 0.
       #bc_fix_strain[i*ndofV+1] = True ; bc_val_strain[i*ndofV+1] = 0.
    if x[i]>(Lx-eps):
       bc_fix_strain[i*ndofV  ] = True ; bc_val_strain[i*ndofV  ] = 0.
       #bc_fix_strain[i*ndofV+1] = True ; bc_val_strain[i*ndofV+1] = 0.
    if y[i]<eps:
...
####################################################################
############# Computational Internal Strains Method ################
####################################################################


Kappa_mat = np.zeros((NfemV,NfemV),dtype=np.float64)
f_rhs = np.zeros(NfemV,dtype=np.float64)
G_mat = np.zeros((NfemV,NfemP),dtype=np.float64) # matrix GT


for iel in range(0,nel):
    Kappa_el=np.zeros((m*ndofV,m*ndofV),dtype=np.float64)
    beta_el=np.zeros((m*ndofV,m),dtype=np.float64)
    d_el=np.zeros((m,m*ndofV),dtype=np.float64)
    G_el=np.zeros((m*ndofV,1),dtype=np.float64)
    f_el =np.zeros((m*ndofV),dtype=np.float64)
    ...
    for iq in [-1, 1]:
        for jq in [-1, 1]:
            ...
            #Need to figure out what Kappa should look like
            #And the best way to implement that boundary conditions

            #How indented does this next bit need to be? Genuinely not sure
            for i in range(0,m*ndofV):
              if (i%2==0):
                beta_el[i,0]=N[int(i/2)]
                beta_el[i,1]=0
                beta_el[i,2]=0
                beta_el[i,3]=N[int(i/2)]

                d_el[0,i]=dNdx[int(i/2)]
                d_el[1,i]=dNdy[int(i/2)]
                d_el[2,i]=0
                d_el[3,i]=0

              else:
                beta_el[i,0]=0
                beta_el[i,1]=-N[int((i-1)/2)]
                beta_el[i,2]=N[int((i-1)/2)]
                beta_el[i,3]=0

                d_el[0,i]=0
                d_el[1,i]=0
                d_el[2,i]=dNdx[int((i-1)/2)]
        ...
        for i1 in range(0,ndofV):
            ...
            for k2 in range(0,m):
                for i2 in range(0,ndofV):
                    jkk=ndofV*k2          +i2
                    m2 =ndofV*icon[k2,iel]+i2
                    Kappa_mat[m1,m2]+=Kappa_el[ikk,jkk]
            f_rhs[m1]+=f_el[ikk] 
            G_mat[m1,iel]+=G_el[ikk,0]
...
for i in range(0, NfemV):
    if bc_fix_strain[i]:
       Kappa_matref = Kappa_mat[i,i]
       for j in range(0,NfemV):
           f_rhs[j]-= Kappa_mat[i, j] * bc_val_strain[i]
           Kappa_mat[i,j]=0.
           Kappa_mat[j,i]=0.
           Kappa_mat[i,i] = Kappa_matref
       f_rhs[i]=Kappa_matref*bc_val_strain[i]

matfile=open("Kappa.dat","w")
for i in range(0,NfemTr):
   for j in range(0,NfemTr):
       if abs(M_prime[i,j])>1e-16:
          matfile.write(" %d %d %e \n " % (i,j,Kappa_mat[i,j]))
matfile.close()
np.savetxt("rhs_strain.dat",f_rhs)
...
sol = sps.linalg.spsolve(sps.csr_matrix(Kappa_mat),f_rhs)
...
exx_direct,exy_direct=np.reshape(sol,(nnp,2)).T

eyy_direct=-exx_direct
