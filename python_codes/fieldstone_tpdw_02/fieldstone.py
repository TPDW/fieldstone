...
def sigma_xx(x,y):
  return x**3*y**2 + 3*x**2 + 3*x*y + 3*x - 3*y**2 - y -1./3.

def sigma_xy(x,y):
  return x**2/2 - 6*x*y - x - y**2/2 - y

def sigma_yy(x,y):
  return x**3*y**2 - 3*x**2 - x*y - x + 3*y**2 + 3*y -7./3.
...
for i in range(0, nnp):
    ...
    if x[i]>(Lx-eps):
       ...
       on_bd[i,1]=True
    if y[i]<eps:
       bc_fix[i*ndofV+1] = True ; bc_val[i*ndofV+1] = velocity_x(x[i],y[i])
       bc_fix[i*ndofV+1] = True ; bc_val[i*ndofV+1] = velocity_y(x[i],y[i])
       on_bd[i,2]=True
    if y[i]>(Ly-eps):
       bc_fix[i*ndofV+1] = True ; bc_val[i*ndofV+1] = velocity_x(x[i],y[i])
...
vtufile.write("</DataArray>\n")
#--
# vtufile.write("<DataArray type='Float32' NumberOfComponents='3' Name='sigma' Format='ascii'> \n")
# for i in range(0,nnp):
#     vtufile.write("%10e %10e %10e \n" %(exxn1[i]+q1[i],eyyn1[i]+q1[i],exyn1[i]))
#  vtufile.write("</DataArray>\n")
