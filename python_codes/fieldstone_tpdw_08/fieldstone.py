...
# Penalty Method CBF Custom Biquadratic Benchmark
...
def bx(x, y):
    val=6*x**3 - 18*x**2 - 126*x*y**2 - 84*x*y + 132*x + 18*y**2 + 12*y
    return val
def by(x, y):
    val=126*x**2*y + 42*x**2 - 36*x*y - 12*x - 6*y**3 + 18*y**2 - 72*y - 8
    return val

def velocity_x(x,y):
    val=(-7*x**3/2 + 3*x**2/2 + 2*x)*(-6*y**2 - 4*y + 4)
    return val

def velocity_y(x,y):
    val=(-21*x**2/2 + 3*x + 2)*(2*y**3 + 2*y**2 - 4*y)
    return val

def pressure(x,y):
    val=12*x**4 - 12*x**3 + 12*x**2 + 12*x - 12*y**4 - 8*y**3 + 18*y**2
    return val
...
def sigma_xx(x,y):
  return 16*x-16*y + (4-8*x)*(2-4*y)

def sigma_xy(x,y):
  return 8*(x**2-x-y**2+y)

def sigma_yy(x,y):
  return 16*x-16*y  + (8*y-4)*(2-4*x) 
...
free_slip=[True,True,True,True]
no_slip=[False,False,False,False]

for i in range(4):
  if (free_slip[i] and no_slip[i]):
    print("A boundary cannot be both no slip and free slip")
...
for i in range(0, nnp):
    ...
    if y[i]>(Ly-eps):
      if(no_slip[2]):
        bc_fix[i*ndofV  ] = True ; bc_val[i*ndofV  ] = 0.
      if(no_slip[2] or free_slip[2]):
        bc_fix[i*ndofV+1] = True ; bc_val[i*ndofV+1] = 0.
      on_bd[i,3]=True

    if x[i]<eps:
      if(free_slip[3] or no_slip[3]):
        bc_fix[i*ndofV  ] = True ; bc_val[i*ndofV  ] = 0
      if(no_slip[3]):
        bc_fix[i*ndofV+1] = True ; bc_val[i*ndofV+1] = 0.
      on_bd[i,0]=True
...
use_starting_guess=True

if use_starting_guess:
  u=velocity_x(x,y)
  v=velocity_y(x,y)
...
##############################################################
#Now I want to calculate the elemental velocity derivatives
##############################################################
    ...
    for iq in [-1, 1]:
        for jq in [-1, 1]:
            ...
            for k in range(0, m):
                ...
                dNdy[k]=jcbi[1,0]*dNdr[k]+jcbi[1,1]*dNds[k]
            for k in range(0,m):
              dvdx[iel]+=dNdx[k]*v[icon[k,iel]]
              dvdy[iel]+=dNdy[k]*v[icon[k,iel]]
              dudx[iel]+=dNdx[k]*u[icon[k,iel]]
              dudy[iel]+=dNdy[k]*u[icon[k,iel]]
...
vtufile.write("</DataArray>\n")
#--
vtufile.write("<DataArray type='Float32' Name='dvdx' Format='ascii'> \n")
for iel in range (0,nel):
    vtufile.write("%10e\n" % dvdx[iel])
vtufile.write("</DataArray>\n")
#--
vtufile.write("<DataArray type='Float32' Name='dvdy' Format='ascii'> \n")
for iel in range (0,nel):
    vtufile.write("%10e\n" % dvdy[iel])
vtufile.write("</DataArray>\n")
#--
vtufile.write("<DataArray type='Float32' Name='dudv' Format='ascii'> \n")
for iel in range (0,nel):
    vtufile.write("%10e\n" % dudx[iel])
vtufile.write("</DataArray>\n")
#--
vtufile.write("<DataArray type='Float32' Name='dudy' Format='ascii'> \n")
for iel in range (0,nel):
    vtufile.write("%10e\n" % dudy[iel])
vtufile.write("</DataArray>\n")
#--
...
vtufile.write("</DataArray>\n")
#--
vtufile.write("<DataArray type='Float32' NumberOfComponents='3' Name='velocity(theoretical)' Format='ascii'> \n")
for i in range(0,nnp):
...
for i in range(0,nnp):
    vtufile.write("%10e %10e %10e \n" %(u[i]-velocity_x(x[i],y[i]),v[i]-velocity_y(x[i],y[i]),0.))
vtufile.write("</DataArray>\n")
#--
