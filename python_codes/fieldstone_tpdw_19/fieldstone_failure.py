...
for i in range(nnp):
    if bc_fix[2*i]: 
        n_x=x[i]/np.sqrt(x**2+y**2)
        n_y=y[i]/np.sqrt(x**2+y**2)
        if (sqrt(x[i]**2+y[i]**2) > 1.5):
            txn1 = n_x*sxxn1+n_y*sxyn1
            tyn1 = n_x*sxyn1+n_y*syyn1
        else:
            txn1 = -n_x*sxxn1-n_y*sxyn1
            tyn1 = -n_x*sxyn1-n_y*syyn1
...
for i in range(nnp):
    if is_inner[i]:
        ...
        tx_inner[counter_inner] = txn1[i]
        ty_inner[counter_inner] = tyn1[i]
