...
dudx_find=np.zeros(nnp,dtype=np.float64)
dvdx_find=np.zeros(nnp,dtype=np.float64)
    ...
    for i in range(0,nelx):
        if i<nelx-1 and j<nely-1:
           ...
           for iel_patch in range(4):
              ...
              for k in range(m):
                x_array[k]=x[icon[k,iel]]
                y_array[k]=y[icon[k,iel]]

              if i==0: x_array[2]+=delta_x
              if i==1: x_array[3]+=delta_x
              if i==2: x_array[0]+=delta_x
              if i==3: x_array[1]+=delta_x
              ...
              for iq in [-1, 1]:
                for jq in [-1, 1]:
                    ...
                    jcb = np.zeros((2,2),dtype=np.float64)
                    for k in range(0,m):
                        # print(x_array[k],x[icon[k,iel]])
                        # print(y_array[k],y[icon[k,iel]])

                        jcb[0, 0] += dNdr[k]*x_array[k]
                        jcb[0, 1] += dNdr[k]*y_array[k]
                        jcb[1, 0] += dNds[k]*x_array[k]
                        jcb[1, 1] += dNds[k]*y_array[k]
           ...
           dudx_find[node]  = (vx_delta-vx_reconstructed[node])/delta_x
           dvdx_find[node]  = (vy_delta-vy_reconstructed[node])/delta_x
...
if visu==1:
       ...
       vtufile.write("</DataArray>\n")
       #--
       vtufile.write("<DataArray type='Float32' NumberOfComponents='1' Name='dudx_find' Format='ascii'> \n")
       for i in range(0,nnp):
           vtufile.write("%10e \n" % dudx_find[i])#-u[i]))
       vtufile.write("</DataArray>\n")
       #--
       vtufile.write("<DataArray type='Float32' NumberOfComponents='1' Name='dvdx_find' Format='ascii'> \n")
       for i in range(0,nnp):
           vtufile.write("%10e \n" % dvdx_find[i])#-u[i]))
       vtufile.write("</DataArray>\n")
       #--
