...
#This is the bit where I give up on the solve, and just do the CBF on a prescribed velocity field
...
def sigma_xx(x,y):
  return -12*x**4 + 12*x**3 - 12*x**2 - 12*x + 12*y**4 + 8*y**3 - 18*y**2 + (-21*x**2/2 + 3*x + 2)*(-6*y**2 - 4*y + 4)

def sigma_xy(x,y):
  return (-21*x + 3)*(2*y**3 + 2*y**2 - 4*y)/2 + (-12*y - 4)*(-7*x**3/2 + 3*x**2/2 + 2*x)/2
...
for iel in range(nel):
  p[iel]=pressure(x[icon[0,iel]]+hx/2,y[icon[0,iel]]+hy/2)  
